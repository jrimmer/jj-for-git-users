...
- To confirm which backend a repo uses, `jj util backend name` prints the current commit backend.
...
| Show commit(s) | `git show REV...` | `jj show REV...` |
...
```toml
[aliases]
l = { doc = "Show ancestors of the working-copy commit", definition = ["log", "-r", "::@"] }
```
